...
export function ChallengeForm({ grantId, milestoneIndex, challengeBond }: { grantId: string; milestoneIndex: bigint; challengeBond: bigint }) {
  const wallet = useWallet(); const transaction = useTransaction(); const [counterEvidence, setCounterEvidence] = useState("");
  const submit = async () => { if (!counterEvidence.trim() || wallet.status !== "connected" || !wallet.account || !wallet.provider) return; const client = await createWriteClient(wallet.provider, wallet.account); const contract = createAidTrailContract({ address: process.env.NEXT_PUBLIC_AIDTRAIL_CONTRACT_ADDRESS, writeClient: client }); await transaction.execute(() => contract.challengeMilestone({ grantId, milestoneIndex, counterEvidenceJson: counterEvidence, readback: async () => true })); };
