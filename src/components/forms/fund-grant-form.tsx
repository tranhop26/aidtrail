...
export function FundGrantForm({ grantId, remaining }: { grantId: string; remaining: bigint }) {
  ...
  const fund = async () => { if (!valid || wallet.status !== "connected" || !wallet.account || !wallet.provider) return; const client = await createWriteClient(wallet.provider, wallet.account); const contract = createAidTrailContract({ address: process.env.NEXT_PUBLIC_AIDTRAIL_CONTRACT_ADDRESS, writeClient: client }); await transaction.execute(() => contract.fundGrant({ grantId, amount: BigInt(amount), readback: async () => true })); };
