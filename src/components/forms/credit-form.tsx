...
import { creditReadback } from "../../lib/workflow-readbacks";
...
export function CreditForm({ credit = 0n }: { credit?: bigint }) {
  ...
  const write = async (kind: "deposit" | "withdraw") => { if (wallet.status !== "connected" || !wallet.account || !wallet.provider || (kind === "deposit" && (!/^\d+$/.test(amount) || BigInt(amount) === 0n))) return; const client = await createWriteClient(wallet.provider, wallet.account); const contract = createAidTrailContract({ address: process.env.NEXT_PUBLIC_AIDTRAIL_CONTRACT_ADDRESS, readClient: createReadClient(), writeClient: client }); await transaction.execute(() => kind === "deposit" ? contract.depositChallengeCredit({ amount: BigInt(amount), readback: creditReadback(contract, wallet.account!, credit) }) : contract.withdrawCredit({ readback: creditReadback(contract, wallet.account!, credit) })); };
