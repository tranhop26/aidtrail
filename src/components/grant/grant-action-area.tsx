...
export function GrantActionArea({ grant, milestone, milestones }: { grant: Grant; milestone: Milestone; milestones: readonly Milestone[] }) {
  ...
  const act = async (action: Parameters<NonNullable<React.ComponentProps<typeof RoleActions>["onAction"]>>[0]) => {
    ...
    if (action === "withdraw_credit") { const account = wallet.account; const before = await contract.readCredit(account); if (before.availability !== "available") throw new Error(before.reason); if (before.data === 0n) return setCreditRead({ account, value: 0n }); const result = await transaction.execute(() => contract.withdrawCredit({ readback: creditReadback(contract, account, before.data) })); if (result.phase === "success" || result.phase === "readback_mismatch") await refreshCredit(account); }
  };
