...
describe("GrantActionArea connected-wallet credit workflow", () => {
  ...
  it("loads deposited contract credit and enables challenge and withdrawal", async () => {
    ...
    contractState.credit = 5n;
    await act(async () => { accountsChanged?.([]); await new Promise((resolve) => setTimeout(resolve, 0)); });
    await act(async () => { accountsChanged?.([account]); await new Promise((resolve) => setTimeout(resolve, 0)); });
    expect(Array.from(container.querySelectorAll("button")).find((button) => button.textContent === "Withdraw refundable credit")?.disabled).toBe(false);
    let resolveStalePreflight: ((value: string) => void) | undefined;
    const stalePreflight = new Promise<string>((resolve) => { resolveStalePreflight = resolve; });
    contractState.readPlan = [stalePreflight, "0"];
    await act(async () => { accountsChanged?.([]); await new Promise((resolve) => setTimeout(resolve, 0)); });
    await act(async () => { accountsChanged?.([account]); await new Promise((resolve) => setTimeout(resolve, 0)); });
